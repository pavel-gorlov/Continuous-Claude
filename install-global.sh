...
mkdir -p "$GLOBAL_DIR/scripts/core"
mkdir -p "$GLOBAL_DIR/scripts/mcp"

# Copy root-level MCP scripts (your fork's scripts/)
...
# Copy .claude/scripts/*.sh (shell utilities)
cp "$SCRIPT_DIR/.claude/scripts/"*.sh "$GLOBAL_DIR/scripts/" 2>/dev/null || true
cp "$SCRIPT_DIR/.claude/scripts/"*.py "$GLOBAL_DIR/scripts/" 2>/dev/null || true

# Copy .claude/scripts/core/ (store_learning, artifact_*, etc.)
if [ -d "$SCRIPT_DIR/.claude/scripts/core" ]; then
    cp -r "$SCRIPT_DIR/.claude/scripts/core/"* "$GLOBAL_DIR/scripts/core/" 2>/dev/null || true
    echo "  → Copied core scripts (store_learning, artifact_index, etc.)"
fi

# Copy opc/scripts/mcp/ (upstream MCP tools with Ollama embedding)
if [ -d "$SCRIPT_DIR/opc/scripts/mcp" ]; then
    cp -r "$SCRIPT_DIR/opc/scripts/mcp/"* "$GLOBAL_DIR/scripts/mcp/" 2>/dev/null || true
    echo "  → Copied upstream MCP tools"
fi

# Copy opc/scripts/core/db/ (embedding_service with Ollama)
if [ -d "$SCRIPT_DIR/opc/scripts/core/db" ]; then
    mkdir -p "$GLOBAL_DIR/scripts/core/db"
    cp -r "$SCRIPT_DIR/opc/scripts/core/db/"* "$GLOBAL_DIR/scripts/core/db/" 2>/dev/null || true
    echo "  → Copied embedding service (Ollama support)"
fi
